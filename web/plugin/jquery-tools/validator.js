...
(function($) {	
	...
	v = $.tools.validator = {
		...
		/* The Error Messages */
		messages: {
			"*": { en: "Please correct this value", zh:"请输入正确的值！" }		
		},
		...
		'default' : [
			...
			function(errs) {
				...
				$.each(errs, function(i, err) {
					...
					// add by virgil.chen 20120926 for set focus to the first input
					if (i == 0) {
						conf.firstErrorInput = input ;
						//input.focus() ;
					}
					...
					// zIndex property added by virgil.chen @ 2012-09-26 for visibel
					msg.css({ visibility: 'visible', position: 'absolute', top: pos.top, left: pos.left,zIndex:(input.zIndex() + 100) })
						.fadeIn(conf.speed);
	...
	/* sperial selectors */
	$.each("email,url,number,integer".split(","), function(i, key) {
	...
	*/
	...
	/** add by Virgil.Chen **/
	v.fn(":integer", "Please enter a integer value.", function(el, v) {
		return /^-?[0-9]*$/.test(v);			
	});
	/** end add **/
	...
	function Validator(inputs, form, conf) {		
		...
		// added by virgil.chen 20120926 for change active-style
        $(inputs).each(function (i, elem) {
        	...
        	if (elemVal != "" && elemVal.substring(0,1) == "{") {// this is a template
        		return ;
        	}
        	...
        	if (!conf.firstInput) {
        		conf.firstInput = elem ;
        		//conf.firstInput.focus() ;
        		//nextFocus(conf.firstInput) ;
        	}
        	
        	if (previewInput != null) {
        		previewInput.nextInput = elem ;
        	}
        	previewInput = elem ;
        	currentInput = elem ;

            //$(elem).on("focus",function() {$(elem).addClass("active");}) ;
            //$(elem).on("blur", function() {$(elem).removeClass("active");}) ;
        	$(elem).off(".validate") ;
            $(elem).on("keydown.validate", function(_event) {
            	var event = $.browser.msie?window.event:_event ;
            	...
            	if(event.keyCode == 13){
            		if (typeof(elem.enter) != "undefined") {
            			elem.enter(event) ;
            			return ;
            		}
            		...
            		if ($.browser.msie) {       
            			...
            			nextFocus(elem.nextInput) ;
            		}
            	} 
            });
        }) ;

        currentInput.nextInput = conf.firstInput ;
        currentInput.isLast = true ;
        
        
        function nextFocus(nextElem) {

			var focusableInput = nextElem ;
			for (var i = 0 ; i < inputs.length ; i ++) {// 找出可以获取焦点的input
				if (focusableInput.disabled) {
					focusableInput = focusableInput.nextInput ;
					continue ;
				}
				if (focusableInput.style.display == "none") {
					focusableInput = focusableInput.nextInput ;
					continue ;
				}
				
				try {// maybe parent is not visible
